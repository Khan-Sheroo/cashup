...
from cashup.models import (
    Staff, StaffDocument, StaffFolder, CashUp, CardMachineBatch, WaiterTip, AppSettings, DailySummary,
)
...
def _default_staff_file_label(filename: str) -> str:
    """Build a readable label from a filename (no snake/kebab casing)."""
...
def _staff_folders_payload(staff_id: int) -> list[dict]:
    folders = (
        StaffFolder.query
...
def _resolve_folder_for_staff(staff_id: int, folder_id):
    """Return folder or None; raises ValueError if folder_id invalid for staff."""
...
def staff_files(staff_id):
    """Dedicated page for a staff member's uploaded files."""
    staff = Staff.query.get_or_404(staff_id)
    ...
    if folder_filter in ('', 'all', None):
        # Filed documents first (grouped by folder name), Unfiled last
        documents = sorted(
            documents,
            key=lambda d: (
...
def api_list_staff_files(staff_id):
    """JSON list of documents (and folders) for a staff member."""
...
def api_update_staff_folder(folder_id):
    """Rename a folder (JSON POST for simplicity)."""
...
def api_delete_staff_folder(folder_id):
    """Delete a folder; files are moved to Unfiled."""
    folder = StaffFolder.query.get_or_404(folder_id)
...
def api_move_staff_file(doc_id):
    """Move a document into a folder, or to Unfiled when folder_id is null."""
...
def api_upload_staff_file(staff_id):
    """Upload one or more documents/images for a staff member.

    Accepts multipart fields:
    - files / labels (preferred multi-upload; parallel lists)
    - file / label (legacy single upload)
    - folder_id (optional; assign all uploads to this folder)
    """
    Staff.query.get_or_404(staff_id)
    ...
    try:
        target_folder = _resolve_folder_for_staff(
            staff_id,
...
def download_staff_file(doc_id):
    """Download or inline-view a staff document."""
    doc = StaffDocument.query.get_or_404(doc_id)
...
def delete_staff_file(doc_id):
    """Delete a staff document (JSON or form)."""
    doc = StaffDocument.query.get_or_404(doc_id)
