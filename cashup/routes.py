...
from cashup.models import Staff, CashUp, CardMachineBatch, WaiterTip
...
def persist_cashup_drafts(form):
    ...
    for i, staff_id_raw in enumerate(entries):
        ...
        if gross_tip > 0:
            ...
            tip_amount = gross_tip - cc_commission - combined_tips
        else:
            cc_commission = Decimal('0')
            combined_tips = Decimal('0')
            tip_amount = gross_tip
        ...
        if existing_cashup and not existing_cashup.submitted:
            ...
            existing_cashup.combined_tips = combined_tips
...
def new_cashup():
    ...
    total_combined_tips = sum(float(c.combined_tips) for c in cashups)
    ...
    draft_total_combined_tips = sum(float(c.combined_tips) for c in draft_cashups)
    ...
    day_total_combined_tips = total_combined_tips + draft_total_combined_tips
...
def submit_cashup():
    """Submit draft cashups for the selected date, moving them to history"""
    date_str = request.form.get('submit_date') or request.form.get('date')
    ...
    try:
        ...
        # Find all draft cashups for this date
        ...
        # Mark all as submitted; tips sheet already synced on save/autosave
        for cashup in draft_cashups:
            cashup.submitted = True
        
        # Final tip sync for the day
        try:
            db.session.commit()
        ...
        # Redirect back to the same date to show updated history
        return redirect(url_for('cashup.new_cashup', date=date_str))
        ...
        flash(f'Error submitting cashups: {str(e)}', 'error')
        db.session.rollback()
        return redirect(url_for('cashup.new_cashup', date=date_str if date_str else None))
